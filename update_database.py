...
import sys
# from nlp import spacy_load_ner
# from nlp import tweet_categorisation_load
import nlp
...
def delete_old_transfers(db, time_days):
    ...
    WHERE
        latest_sources.latest_timestamp < {0}""".format(int(time.time()) - time_seconds)
...
def update_database(mysql_user, mysql_password, mysql_port, time_wait):
    # categorisation_model = tweet_categorisation_load.TweetCategorisationModel()
    # ner_model = spacy_load_ner.NERModel()
    ...
    for source in sources: #source: [source_id, text, timestamp]
        ...
        #transfer_data = nlp.interpret_source(source[1], categorisation_model, ner_model) 
        print(transfer_data)
        sys.stdout.flush()
        if transfer_data[0]:
            try:
