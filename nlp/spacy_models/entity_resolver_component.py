...
Doc.set_extension("resolved", default={}, force=True)
msg = Printer()

@Language.factory(
    "entity_resolver",
    requires=["doc.ents", "token.ent_iob", "token.ent_type"],
    assigns=["doc._.resolved"],
...
def make_entity_resolver(
    nlp: Language, name: str, model: Model, *, threshold: float
):
    return EntityResolver(nlp.vocab, model, name, threshold=threshold)

class EntityResolver(TrainablePipe):
    def __init__(
        self,
        vocab: Vocab,
        model: Model,
    ...
    def predict(self, docs: Iterable[Doc]) -> Floats2d:
        ...
        if total_instances == 0:
            ...
            return self.model.ops.alloc2f(0, 0)
    ...
    def set_annotations(self, docs: Iterable[Doc], scores: Floats2d) -> None:
        ...
        for doc in docs:
            for (e1, e2) in get_instances(doc):
                ...
                if offset not in doc._.resolved:
                    doc._.resolved[offset] = 0.0
                #for j, label in enumerate(self.labels):
                doc._.resolved[offset] = scores[c, 0]
                c += 1
    ...
    def update(
        ...
        *,
        ...
        total_instances = 0
        for eg in examples:
            total_instances += len(self.model.attrs["get_instances"](eg.reference))
        if total_instances == 0:
    ...
    def _examples_to_truth(self, examples: List[Example]) -> Optional[numpy.ndarray]:
        ...
        truths = numpy.zeros((nr_instances, 1), dtype="f")
        c = 0
        for i, eg in enumerate(examples):
            for (e1, e2) in self.model.attrs["get_instances"](eg.reference):
                score = eg.reference._.resolved.get((e1.start, e2.start), 0.0)
                # for j, label in enumerate(self.labels):
                truths[c, 0] = score
                c += 1
...
def score_relations(examples: Iterable[Example], threshold: float) -> Dict[str, Any]:
    ...
    for example in examples:
        gold = example.reference._.resolved
        pred = example.predicted._.resolved
        for key, value in pred.items():
            #gold_labels = [k for (k, v) in gold.get(key, {}).items() if v == 1.0]
            gold_label = gold.get(key, 0.0)
